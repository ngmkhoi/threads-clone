...
import {updatePostInteractions} from "@/features/posts/postsSlice.js";
import {cn} from "@/lib/utils.js";
import AnimatedCounter from "@/components/Post/PostCard/components/InteractionBar/components/AnimatedCounter/index.jsx";
...
export default function Like({ count, post }) {
    ...
    const handleClick = async () => {
        if(isAuthenticated) {
            ...
            dispatch(updatePostInteractions({
                postId: post.id,
    ...
    return (
        <>
            <button
                className={cn(
                    "flex items-center gap-1 transition-colors cursor-pointer p-1.5",
